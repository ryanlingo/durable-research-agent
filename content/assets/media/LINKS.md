# Media links

| Date | Item | Plan ID | Link / path |
|------|------|---------|-------------|
| 2026-07-31 | Showcase crash demo (video, ~24s) | P3.2 | `content/assets/media/2026-07-31-showcase-crash-demo.mp4` |
| 2026-07-31 | Showcase crash demo (webm source) | P3.2 | `content/assets/media/2026-07-31-showcase-crash-demo.webm` |
| 2026-07-31 | Showcase dual pipelines idle | P3.3 | `content/assets/media/2026-07-31-showcase-idle.png` |
| 2026-07-31 | Showcase mid-run | P3.3 | `content/assets/media/2026-07-31-showcase-mid-run.png` |
| 2026-07-31 | Post-crash full page (savings) | P3.3 | `content/assets/media/2026-07-31-showcase-comparison.png` |
| 2026-07-31 | Post-crash savings panel | P3.3 | `content/assets/media/2026-07-31-showcase-comparison-panel.png` |
| 2026-07-31 | Temporal Web UI | P3.3 | `content/assets/media/2026-07-31-temporal-web-ui.png` |
| 2026-07-31 | Crash-lab talk deck (pptx) | deck | `content/decks/2026-07-31-durable-agents-crash-lab.pptx` |

Re-record demo:

```bash
python -m ui.app   # if not running
python content/assets/media/record_showcase_demo.py
```

Upload the MP4 to Loom (or YouTube) if you want a shareable hosted link; the file is the visual demo.
